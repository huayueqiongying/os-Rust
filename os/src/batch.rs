...
pub struct AppManager { inner: RefCell<AppManagerInner> }
...
impl AppManagerInner {
    ...
    unsafe fn load_app(&self, app_id: usize) {
        ...
        let src = unsafe { core::slice::from_raw_parts(self.app_start[app_id] as *const u8, self.app_start[app_id+1]-self.app_start[app_id]) };
...
lazy_static! {
    pub static ref APP_MANAGER: AppManager = AppManager {
        inner: RefCell::new({
            ...
            let arr = unsafe { core::slice::from_raw_parts(num_app_ptr.add(1), num_app+1) };
            app_start[..=num_app].copy_from_slice(arr);
...
pub fn run_next_app() -> ! {
    let app_id = APP_MANAGER.inner.borrow().get_current_app();
    unsafe { APP_MANAGER.inner.borrow().load_app(app_id); }
    APP_MANAGER.inner.borrow_mut().move_to_next_app();

    unsafe extern "C" { fn __restore(addr: usize); }
    let cx = TrapContext::app_init_context(APP_BASE_ADDRESS, USER_STACK.get_sp());
    let cx_ptr = KERNEL_STACK.push_context(cx);
    unsafe { __restore(cx_ptr as *mut TrapContext as usize); }
    loop {}
}
